...
# Adapted from hhttps://github.com/PaddlePaddle/PaddleHelix/blob/dev/apps/drug_target_interaction/sign/preprocess_pdbbind.py
"""
...
def pocket_atom_num_from_mol2(name, path):
    n = 0
    with open('%s/%s/%s_ligand.mol2' % (path, name, name)) as f:
        for line in f:
...
def gen_feature(path, name, featurizer):
    #featurizer.FEATURE_NAMES 名字 ['B', 'C', 'N', 'O', 'P', 'S', 'Se', 
    # 'halogen', 'metal', 'hyb', 
    # 'heavydegree', 'heterodegree', 'partialcharge', 
...
def process_dataset(core_path, refined_path, general_path, dataset_name, output_path, cutoff):
    core_set_list = [x for x in os.listdir(core_path) if len(x) == 4]

    # refined_set_list = [x for x in os.listdir(refined_path) if len(x) == 4]
    # general_set_list = [x for x in os.listdir(general_path) if len(x) == 4]
    path = refined_path
    path1 = general_path
    ...
    for name in tqdm(os.listdir(path1)):
        if len(name) != 4:
            continue
        processed_dict1[name] = gen_feature(path1, name, featurizer)    

    # interaction features
    processed_dict = pairwise_atomic_types(path, processed_dict, atom_types, atom_types_)
    processed_dict1 = pairwise_atomic_types(path1, processed_dict1, atom_types, atom_types_)

    # load pka (binding affinity) data
    pk_dict = load_pk_data(path+'index/INDEX_refined_data.2016')
    pk_dict1 = load_pk_data(path1 + 'index/INDEX_general_PL_data.2016')
    data_dict = processed_dict
    data_dict1 = processed_dict1

    for k,v in processed_dict.items():
        v['pk'] = pk_dict[k]
        data_dict[k] = v

    for k,v in processed_dict1.items():
        v['pk'] = pk_dict1[k]
        data_dict1[k] = v    

    refined_id, refined_data, refined_pk = [], [], []
    core_id, core_data, core_pk = [], [], []
    general_id, general_data, general_pk = [], [], []
    ...
    for k, v in tqdm(data_dict1.items()):
       
        lf_size, pf_size = v['lig_fea'].shape[0], v['pock_fea'].shape[0]
         
        _, lf_nd = np.where(v['lig_fea'][:,:9] > 0)
        _, pf_nd = np.where(v['pock_fea'][:,:9] > 0)
        if lf_size != len(lf_nd) or pf_size != len(pf_nd):
            continue 
        ligand = (v['lig_fea'], v['lig_co'], v['lig_atoms'], v['lig_eg'])
        pocket = (v['pock_fea'], v['pock_co'], v['pock_atoms'], v['pock_eg'])
        ...
        general_id.append(k)        
        general_data.append(graph)
        general_pk.append(pk) 
    # split train and valid
    train_idxs, valid_idxs = random_split(len(refined_data), split_ratio=0.734, seed=2020, shuffle=True)
    #train_add_idxs = 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path_core', type=str,default = '/data/wujq/16core/')
    parser.add_argument('--data_path_refined', type=str,default = '/data/wujq/v2016/')
    parser.add_argument('--data_path_general', type=str,default = '/data/wujq/general/')
    parser.add_argument('--output_path', type=str, default='/data/wujq/')
    parser.add_argument('--dataset_name', type=str, default='g2016')
    parser.add_argument('--cutoff', type=float, default=5.)
    args = parser.parse_args()
    process_dataset(args.data_path_core, args.data_path_refined, args.data_path_general,args.dataset_name, args.output_path, args.cutoff)
